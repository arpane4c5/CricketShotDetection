...
def create_labels(srcFolderPath, destFolderPath, stop='all'):
    ...
    for sf in sfp_lst:
        ...
        if os.path.isdir(sub_src_path):
            ...
            for vid in vfiles:
                if os.path.isfile(os.path.join(sub_dest_path, vid)):
                    continue
...
# create labels for single video, params are srcVideo and dest xml filepath
    ...
    while cap.isOpened():
        ...
        if ret==True:
            ...
            if direction == 1:
                ...
                i = i+1
                videoLabels.append((i-1,i))
            else:
                i -=1
        else:
            break

    print("No. of boundaries in video : "+str(len(videoLabels)))
    print(videoLabels)
